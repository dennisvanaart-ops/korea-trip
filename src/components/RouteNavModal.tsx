...
import { buildRouteLinks } from "@/lib/tripHelpers";

interface Props {
  origin: string;
  destination: string;
  /** Human-readable label, e.g. "Seoul → Sokcho" */
  label: string;
...
export function RouteNavModal({ origin, destination, label, duration, onClose }: Props) {
    ...
    <>
      ...
      >
        ...
        <div className="px-5 pb-4">
          ...
          <p className="text-xs text-gray-400 mb-4">Route openen in</p>

          <div className="space-y-2">
            {/* Google Maps — primary, shows full route */}
            <RouteItem
              href={links.google}
              label="Google Maps"
              sublabel={`${originCity} → ${destCity} · rijroute`}
              primary
              icon={GoogleIcon}
            />
